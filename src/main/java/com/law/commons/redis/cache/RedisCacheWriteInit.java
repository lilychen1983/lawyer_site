 ...
 * 修改记录：
 */
public class RedisCacheWriteInit {
...
//	private static final Integer MAXWAIT = 1000;
	...
	//	jedisPoolConfig.numTestsPerEvictionRun = -1;
		...
		String host = PpsConfig.getString("redis.data.write.server.host");
		Integer port = Protocol.DEFAULT_PORT;
		if (PpsConfig.getString("redis.data.write.server.port") != null)
			port = Integer.valueOf(PpsConfig
					.getString("redis.data.write.server.port"));
